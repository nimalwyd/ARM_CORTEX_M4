/*
  ...
  *DESCRIPTION    :  This project is  for seting up an adc in stm32f3 board, which will recive analog readings from a temperature sensor,
  and will display leds of different colours based on the temperature range.CUbemX is used to configure the ADC.
...
int main(void)
{
  ...
  // Nothing to be done in the polling method as the ADC value is read and LED is lit using the Timer
